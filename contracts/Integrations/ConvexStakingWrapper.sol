...
contract ConvexStakingWrapper is ERC20, ReentrancyGuard {
	...
	function addRewards() public {
		...
		if (rewards.length == 0) {
			...
			rewards.push(RewardType({ reward_token: cvx, reward_pool: address(0), reward_integral: 0, reward_remaining: 0 }));
		...
		for (uint256 i = 0; i < extraCount; i++) {
			...
			if (extraToken == cvx) {
				...
				rewards.push(
					RewardType({ reward_token: extraToken, reward_pool: extraPool, reward_integral: 0, reward_remaining: 0 })
				);
