import { Wallet as ZksyncWallet, utils } from "zksync-web3"
...
export class CoreDeployer {
	...
	deployerWallet: EthersWallet | ZksyncWallet
	...
	constructor(hre: HardhatRuntimeEnvironment, targetNetwork: DeploymentTarget) {
		...
		this.hre = hre
		if (this.isZksyncDeployment()) {
			this.deployerWallet = new ZksyncWallet(process.env.DEPLOYER_PRIVATEKEY)
		} else {
			this.deployerWallet = new EthersWallet(process.env.DEPLOYER_PRIVATEKEY, this.hre.ethers.provider)
		}
	}
	...
	isZksyncDeployment = () => this.targetNetwork == DeploymentTarget.ZkSyncTestnet
	 ...
	 * If any of the contracts have already been deployed and contain a matching entry in a JSON
