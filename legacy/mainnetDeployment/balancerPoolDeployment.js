...
async function main() {
  ...
  */
  ...
  th.logBN('Initial VUSD', USDV_balance);
  ...
  const txApproveUSDV = await VUSD.approve(VAULT, USDV_balance);
  const approveUSDVReceipt = await txApproveUSDV.wait();
  console.log('Approve VUSD gas: ', approveUSDVReceipt.gasUsed.toString());
